...
def filter_tracked_stocks_details(symbol, detailed):
    con = sql.connect(config.DATABASE) 
    cur = con.cursor()
        
    if symbol:
        cur.execute("SELECT * FROM stock_tracker WHERE stock_id = (SELECT id from stock WHERE symbol=?)", [symbol]) 
    ...
    for stock_details in tracked_stocks:
        ...
        increase = True if stock_details[3] else False
        decrease = True if stock_details[4] else False
        last_modified = stock_details[5]

        if detailed:
            response = get_stock_quote(symbol)
            if not response:
                return jsonify({"Error": "You are tracking an invalid stock symbol: {symbol}".format(symbol=symbol)}), 404        
            ...
            tracked_stocks_list.append({"symbol": symbol, "name": name, "percent_difference": percent_difference,
                                        "last_modified": last_modified, "alert_on_increase": increase, "alert_on_decrease": decrease, 
                                        "average_cost": average_cost, "percent_to_track_threshold": percent})
        else:
...
def trigger_alert(stocks_increased, stocks_decreased):
    ...
    if stocks_increased:    
        ...
        for stock in stocks_increased:
            increase_alert_message = increase_alert_message + "[" + stock.get("symbol") + "]" + stock.get("name") + " went up " + u'\u2191' + str(stock.get("percent_increase")) + "%\n"

    if stocks_decreased:
        decrease_alert_message = "Stocks decreased: \n"
        for stock in stocks_decreased:
            decrease_alert_message = decrease_alert_message + "[" + stock.get("symbol") + "]" + stock.get("name") + " went down " + u'\u2193' + str(stock.get("percent_decrease")) + "%\n"
...
def get_tracked_stocks():
    """
    Run caculation of percent change for each tracked stock
    Trigger alert if increase or decrease is enabled and percent threshold is met
    Cron will call this function
    """
    tracked_stocks_list = filter_tracked_stocks_details(None, True)
